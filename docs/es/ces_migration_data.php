...
@subsection users_file Arxiu users.csv
 ...
 - UserType: El tipus de compte. adm per administrador, org per organització sense ànim de lucre, ind per individual, fam per compartit, com per empreses, pub per a comptes públics, vir per virtual. L'administrador té més permisos i només n'hi ha un (que jo sàpiga), org, ind, fam i com són iguals a la pràctica (crec). pub és més accessible en el sentit que les transaccions d'aquest compte les pot veure tothom. vir són comptes per comptabilitzar els intercanvis  amb altres xarxes i no pertanyen a ningú.
 - FirstName: Nom
 - Surname: Cognom
 ...
 - Subarea: El CES té el concepte de subarea per dividir la xarxa en vàries regions. Llavors es pot assignar un coordinador per regió que té permisos sobre els comptes d'aquella regió. També es poden buscar ofertes només referents a comptes associats a aquesta regió. Aquest concepte (de moment) no existeix a l'integralCES.
DefaultSub. 0=fals, -1=cert. Si cert, aquest usuari veurà per defecte només la informació relacionada amb la seva subàrea. Els usuaris poden veure informació de totes les subàrees o d'una concreta utilitzant el filtre de subàrea en la pàgina principal del seu compte.
 ...
 - InvNo: ?
 - OrdNo: ?
 - Coord: Fals si 0, cert si -1. Si cert, aquest usuari és coordinador local, cosa que significa que té permisos per administrar els comptes i transaccions dels usuaris de la seva Subarea.
 - CredLimit: Límit de crèdit. La màxima quantitat de moneda que aquest compte pot tenir. 0 per sense limit.
 - DebLimit: Límit de dèbit. El màxim deute que aquest compte pot tenir. 0 per sense límit.
 - LocalOnly. 0 per fals, -1 per cert. Si cert, aquest usuari només pot fer transaccions amb comptes de la mateixa xarxa (no pot fe canvi de moneda).
 - Notes: Notes per a l'administració. Per defecte conté les dades originals del registre, en fomat text,
 - Lang: idioma en tres lletres. No segueix l'estàndard ISO. eng per anglès, cat per català, spa per castellà.
 - Photo: Nom de l'arxiu de la foto de perfil. Però l'arxiu en sí no és accessible.
 ...
 - HideArea: ? No estic segur de si significa amagar la SubArea, però la interfície no ho deixa escollir.
 ...
 - SubsDue: Data del proper pagament de subscripció, només per xarxes que ho tinguin activat.
 - Closed: Si aquest compte està tancat (no s'esborra per mantenir l'enllaç de la taula de transaccions).
 - DateClosed: Si el compte és ancat, data en que que es va tancar el compte.
 - Translate. Si el compte té permisos per a traduir la interfície al seu idioma.
 - Locked. si el compte està bloquejat i per tant no pot efectuar transaccions.
 - Buddy: Número de compte de l'amic. L'amic és el compte que va portar aquest nou compte a la xarxa de moneda social. Hi ha un mecanisme opcional que fa que l'amic rebi un petit % de cada cobrament d'aquest compte.
...
@subsection settings_file Arxiu settings.csv
 ...
 - ExchangeType: tb per bancs del temps, no sé com deu ser per a les xarxes basades en moneda de curs legal.
 - ExchangeDescr: Descripció llarga de la xarxa.
 - Password: Password d'administració de la xarxa.
 - Town: Regió de la xarxa.
 - Logo: Nom de l'arxiu del logo de la xarxa. L'arxiu en sí és inaccessible.
 ...
 - AdminEmail: Mail genèric d'administració de la xarxa que proporciona el CES i que està redireccionat al mail del camp Email, i que és el remitent dels mails automàtics enviats pel sistema.
 - MemSec: Nom del coordinador de membres. El coordinador de membres és una interfície d'administració amb menys privilegis (no pot veure transaccions) però pot crear usuaris, actualitzar-los i també adminstrar les ofertes i demandes.
 ...
 - LevyRate: Percentatge de impost sobre les transaccions que s'aplica en aquesta xarxa.
 ...
 - DefaultExchanges: Separades per salts de línia, els codis de 4 lletres de les xarxes “amigues” que apareixen per defecte al buscar ofertes (no sé si altres coses) després de les ofertes de la xarxa local.
 - Cell: Telèfon mobil de l'administrador.
 - SubscriptionExchange: 0=fals, -1=cert. Cert per aquelles xarxes que requereixen que els seus membres paguin subscripcions. Afegeix un camp (SubsDue) en el registre de l'usuari que diu la data del proper pagament de subscripció.
 - WelcomeLetter: Missatge de benvinguda. Conté els camps dinàmics {New member's name}, {Account #}, {Password}.
 - InviteLetter: Missatge d'invitació.
 - InviteLetterHead: Fragment del títol del missatge d'invitació que va entremig de: [Nom d'usuari] {fragment} [Nom de la xarxa]
 - DoMoney: ? Crec que és per activar la funcionalitat de euros virtuals. Amb aquesta funcionalitat cada compte té un balanç en moneda social i un balanç en euros virtuals. Els euros virtuals són euros dipositats en una caixa comuna i que es transfereixen virtualment d'un usuari a l'altre.
 - ConRedeemRate: ?
 - HidePsw: ? No sé quin password se suposa que amaga.
 - NoDetails: 0=fals, -1=cert. En cert, amaga es balanços i el nombre i quntitat de les transaccions dels usuaris.
...
@subsection trades_file Arxiu trades.csv
 ...
 - RecordID: Codi que identifica la transacció. Sembla el compte que ha efectuat la transacció concatenat amb la data de la transacció.
 - DateEntered: Data que s'ha entrat la transacció.
 - EnteredBy: UID del compte que ha efectuat la transacció. Normalment el venedor, però també pot ser un administrador.
 - Amount: Quantitat.
...
@subsection offers_file Arxiu offers.csv
 ...
 - Remote: Per a ofertes posades per usuaris que no pertanyen a aquesta xarxa, un codi que conté el UID del compte que l'ha posat i (crec) que la data d'expiració i algo més. En aquest cas UID és l'usuari virtual.
